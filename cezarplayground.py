...
class BlogFront(BlogHandler):
    def get(self):
        ...
        if posts==[] or posts is None :
            logging.error("DBQuery read")
            posts = greetings = Post.all().order('-created')
            posts=list(posts)
            memcache.set(key,posts)
            cacheupdate_start=time.time()
            global cacheupdate_start
...
class PostPage(BlogHandler):
    def get(self, post_id):
        key_m=str(post_id)
        post=memcache.get(key_m)
        cacheupdate=time.time()
        if post is None:
            logging.error("DBQuery")
            key = db.Key.from_path('Post', int(post_id), parent=blog_key())
            post = (db.get(key)).render()
            memcache.set(key_m,post)
            post=memcache.get(key_m)
...
#######
        if subject and content:
            ...
            time.sleep(0.5)
            logging.error("DBQuery read")
            posts = greetings =p.all().order('-created')
            posts=list(posts)
...
class BlogJsonHandler(BlogHandler):
    def get(self):
        posts = db.GqlQuery("select * from Post order by created desc limit 15")
        timeformat = '%a %b %d %H:%M:%S %Y'
        pythondict = [{"subject":p.subject,
                      'content':p.content,
                      'created':p.created.strftime(timeformat),
                      'last_modified':p.last_modified.strftime(timeformat)}
                      for p in posts]
...
class PermalinkJsonHandler(BlogHandler):
    def get(self,post_id):
        key = db.Key.from_path('Post', int(post_id), parent=blog_key())
        post=db.get(key)   #Query to lookup the Post with id=post_id 
        if not post:
            self.error(404)
            return
        pythondict = {"subject":post.subject, "content":post.content}
